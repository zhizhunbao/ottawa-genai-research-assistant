...
class DocumentRepository(BaseRepository[Document]):
    ...
    def __init__(self, data_file: str = "backend/monk/documents/documents.json"):
...
class DocumentChunkRepository(BaseRepository[DocumentChunk]):
    ...
    def find_by_page(self, doc_id: str, page_number: int) -> List[DocumentChunk]:
        """Find chunks by document and page number."""
        data = self._load_data()
        chunks = []
        for item in data:
            if item.get('doc_id') == doc_id and item.get('page_number') == page_number:
                chunks.append(self._from_dict(item))
        return chunks 