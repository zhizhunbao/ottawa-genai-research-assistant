...
### 💬 AI Chat & RAG Frontend
Open-source AI chat interfaces with RAG, streaming, and document management — primary references for the Chat module.

- **[rag-web-ui](rag-web-ui)**: ⭐ **最佳业务匹配** — Next.js 14 + TypeScript + shadcn/ui + Tailwind。Key patterns:
  - Chat + RAG + 文档管理一体化（前后端分离架构）
  - SSE streaming responses with real-time feedback
  - 文档上传 → 自动分块 → 向量化 → 检索全流程
  - 多格式支持 (PDF, DOCX, Markdown, Text)
  - 对话内引用 citations + 置信度
  - 多轮上下文对话
- **[lobe-chat](lobe-chat)**: ⭐ 70k+ stars — **UI/UX 最佳参考** (Next.js + Zustand + Ant Design)。Key patterns:
  - 专业级 UI 动画和排版设计
  - Zustand 状态管理模式（与我们一致）
  - 100+ 插件生态系统
  - 多模型适配 (OpenAI, Anthropic, Google, 本地 LLM)
  - 文件上传 + 知识库 RAG
  - TTS/STT 语音交互
  - 多用户管理 + PostgreSQL 持久化
- **[chatbot-ui](chatbot-ui)**: McKay Wrigley 的 ChatGPT 风格聊天 UI (Next.js + Supabase)。Key patterns:
  - 简洁清晰的 Chat 交互设计
  - 会话管理 + 消息持久化
  - SSE streaming 响应
  - 多模型切换
- **[assistant-ui](assistant-ui)**: ⭐ **可组合 Chat 组件原语库** (React + TypeScript)。Key patterns:
  - Composable primitives — 非整体方案，按需组合
  - 内置 streaming + auto-scroll + retry
  - Markdown 渲染 + 代码高亮
  - 附件上传 + 语音输入
  - 集成 AI SDK, LangGraph, Mastra
  - 可直接 `npm install @assistant-ui/react` 引入
