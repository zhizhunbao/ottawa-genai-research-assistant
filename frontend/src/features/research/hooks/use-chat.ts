...
import { chatApi } from '@/features/research/services/chat-api'
import { useChatStream } from '@/features/research/hooks/use-chat-stream'
...
export function useChat() {
  ...
  const sendMessage = useCallback(
    async (content: string, model?: string) => {
      ...
      const session = useResearchStore.getState().sessions.find((s) => s.id === sessionId)
