...
import { ConfidenceDetails } from './confidence-details';
import { QueryMetadataPanel } from './query-metadata-panel';
import { EvaluationScoresDisplay } from './evaluation-scores';
import { InlineCharts } from './inline-charts';
                ...
                <div className="flex flex-col gap-1.5 min-w-0">
                    ...
                    <div className={cn(
                        ...
                        {message.isLoading && !message.content ? (
                            ...
                            <>
                                ...
                                {/* Inline chart visualization — single chart (legacy) */}
                                {message.chart && (
                                ...
                                {/* M11: Multiple inline charts from ResponseEnvelope */}
                                {!message.chart && message.citations && message.citations.length > 0 && (
                                    <InlineCharts
                                        charts={[]}
                    ...
                    {isAssistant && !message.isLoading && !hasError && (
                        <div className="flex items-center gap-2 px-1 flex-wrap">
                            {/* M11: 4-dimensional confidence details */}
                            {message.confidenceScore ? (
                                <ConfidenceDetails score={message.confidenceScore} />
                            ) : message.confidence !== undefined ? (
                                <ConfidenceIndicator confidence={message.confidence} />
                            ) : null}

                            {/* M11: Query metadata panel */}
                            {message.queryMetadata && (
                                <QueryMetadataPanel metadata={message.queryMetadata} />
                            )}

                            {/* M11: Evaluation scores */}
                            {message.evaluation && (
                                <EvaluationScoresDisplay evaluation={message.evaluation} />
                            )}
